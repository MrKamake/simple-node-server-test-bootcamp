...
describe('GET /google', () => {
  it('should redirect to google', done => {
    request(app)
      ...
      .expect(302)
      .expect('Location', 'www.google.com')
      .end(done);
  });
});
...
describe('GET /user/:user_id/token', () => {
  ...
  it('should generate token to valid user', done => {
    ...
    request(app)
      ...
      .end((err, res) => {
        if (err) return done(err);
        expect(res.body).to.eql({ result: 'ok', token: jwt.sign(user, YOUR_SECRET_KEY) });
        done();
      });
  });
});
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        request(app)
          ...
          .end((err, res) => {
            if (err) return done(err);
            expect(res.body).to.eql({ result: 'ok', secret: 'i am secret something' });
            done();
          });
      });
  });
...
describe('With mongoDB database', () => {
  ...
  before(done => {
    ...
    setTimeout(function checkDatabaseConnection() {
      if (db.readyState === 1) {
        return done();
      }
        ...
        .end((err, res) => {
          ...
          request(app)
            ...
            .end(async (err, res) => {
              ...
              const addedArticle = await Article.findOne({ author: 'Douglas Crockford' });
        ...
        .end((err, res) => {
          ...
          request(app)
            ...
            .end(async (err, res) => {
              ...
              const allArticles = await Article.find();
              const updatedArticle = await Article.findOne({ title: 'Hello World' });
